    ...
    <div className="m-4 border-2 border-primary-900 overflow-hidden rounded-md shadow-sm bg-primary-200/30 ">
      <div className="min-h-80 aspect-w-1 aspect-h-1 w-full group-hover:opacity-75 lg:aspect-none lg:h-80 rounded-t-sm overflow-hidden border-b-2 border-primary-900">
        <Link to={`/products/${_id}`}>
          <img alt={name} src={`/images/${image}`} className="h-full w-full object-cover object-center lg:h-full lg:w-full hover:scale-105 active:scale-100 duration-300 transform-gpu transition-all hover:delay-100 active:delay-0" />
        </Link>
      </div>
      ...
      <div className='grid h-44 content-between'>
        ...
        {size && <div className="p-2 font-medium text-primary-100 select-none">{size}</div>}
          <p className="p-2 font-regular text-primary-100 select-none">
            {quantity} in stock
